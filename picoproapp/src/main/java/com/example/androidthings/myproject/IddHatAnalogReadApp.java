...
public class IddHatAnalogReadApp extends SimplePicoPro {
    ...
    @Override
    public void loop() {
        //initiliaze all light pins to 0
        ...
        double timeValue = Double.parseDouble(currentTime);
        ...
        if (shortAvg >= 0.4) {
            if (IrCurrentTime - IrTriggerTime > 500) {
                ...
                if (timeValue<=21.00 && timeValue>12.00){
                    digitalWrite(off,LOW);
                }
            }
        }

        //if its morning and you toggle the device
